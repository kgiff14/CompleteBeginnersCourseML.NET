...
namespace DTBinaryClassification.AppLogic;
...
internal class Predictor
{
    ...
    internal void Predict<T>(AutismInput input, TrainerAbstract<T> trainer) where T : class
    {
        Console.WriteLine($"\n\n*************************************************\n*\t{trainer}\n*\n*");

        string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory.Split(@"\bin")[0], @"Assets\", "csv_result-Autism_Data.csv");
        ...
        var predictionEngine = _mlContext.Model.CreatePredictionEngine<AutismInput, AutismOutput>(_model);
        var prediction = predictionEngine.Predict(input);

        Console.WriteLine($"*");
        Console.WriteLine($"*-------------------------------");
        Console.WriteLine($"*");
        Console.WriteLine($"*\tIs on ASD: {prediction.ASD}");
        Console.WriteLine($"*");
        Console.WriteLine($"*************************************************");
    }

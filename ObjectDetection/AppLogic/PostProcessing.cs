...
/// <summary>
/// Math is provided by https://github.com/BobLd/YOLOv4MLNet/blob/master/YOLOv4MLNet/DataStructures/YoloV4Prediction.cs
/// Originally ported from https://github.com/onnx/models/tree/main/vision/object_detection_segmentation/yolov4#postprocessing-steps
/// </summary>
public class PostProcessing
{
    ...
    public IReadOnlyList<ImageOutput> GetResults(string[] categories)
    {
    ...
    private List<ImageOutput> NMS(List<float[]> postProcesssedBoundingBoxes, string[] categories)
    {
        postProcesssedBoundingBoxes = postProcesssedBoundingBoxes.OrderByDescending(x => x[4]).ToList();
        var resultsNms = new List<ImageOutput>();
        ...
        while (counter < postProcesssedBoundingBoxes.Count)
        {
            ...
            resultsNms.Add(new ImageOutput(result.Take(4).ToArray(), label, confidence));

...
namespace DecisionTree16Personality.AppLogic;
...
internal class Questionaire
{
    ...
    internal PersonalityInput GetPersonalityInput()
    {
        PersonalityInput input = new();
        Type type = typeof(PersonalityInput);

        WelcomeMessage();

        for (var i = 1; i <= type.GetProperties().Length - 2; i++)
        {
            var index = _questions.TryGetValue(i, out var question);
            ...
            while (!VerifyAnswer(answer))
            {
                answer = Console.ReadLine();
            }

            type.GetProperty($"Question{i}")?.SetValue(input, Convert.ToSingle(answer));
        }

        Console.WriteLine("\nThanks For Taking Our Test!");
        Console.WriteLine("We are calulating your results!");
    ...
    private void WelcomeMessage()
    {
        Console.WriteLine("  **************************************");
        Console.WriteLine("  *                                    *");
        Console.WriteLine("  * Welcome to the 16 personality test *");
        Console.WriteLine("  *                                    *");
        Console.WriteLine("  * Instructions:                      *");
        Console.WriteLine("  * Please answer with a number ranging*");
        Console.WriteLine("  * from -3 to 3.                      *");
        Console.WriteLine("  *                                    *");
        Console.WriteLine("  * Fully Agree: 3                     *");
        Console.WriteLine("  * Partially Agree: 2                 *");
        Console.WriteLine("  * Slightly Agree: 1                  *");
        Console.WriteLine("  * neutral -> 0                       *");
        Console.WriteLine("  * Slightly disagree: -1              *");
        Console.WriteLine("  * Partially disagree: -2             *");
        Console.WriteLine("  * Fully disagree: -3                 *");
        Console.WriteLine("  *                                    *");
        Console.WriteLine("  **************************************");
    }
    ...
    private bool VerifyAnswer(string? answer)
    {
        try
        {
            if (answer == null || Convert.ToInt32(answer) < -3 || Convert.ToInt32(answer) > 3)
            {
